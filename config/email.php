...
function sendPasswordResetEmail($recipientEmail, $resetToken) {
    ...
    // Send token and email directly - token is already random/secure
    $resetLink = "http://g2fchristianschool.ct.ws/reset_password.php?token=" . urlencode($resetToken) . "&email=" . urlencode($recipientEmail);
